  ...
  switch (type) {
    ...
    case "toggle":
      return (<details>
            ...
            {value.children?.map((block) => (<Fragment key={block.id}>{renderBlock(block)}</Fragment>))}
          </details>);
    case "child_page":
    ...
    case "code":
      return value.text?.map((t, idx) => {
        ...
        return (<pre key={idx} className={styles.pre}>
    ...
    case "callout":
      ...
      return (<h1 key={"callout" + id} className={styles.callout}>
            {value.icon.emoji}
            {fullText}
          </h1>);
    case "bookmark":
        ...
        <article className={styles.container}>
          ...
          <section>
            {blocks.map((block, idx) => {
              return (<Fragment key={block.id + idx}>{renderBlock(block)}</Fragment>);
            })}
            <Link href="/">
...
export const getStaticProps = async (context) => {
  ...
  const childBlocks = await Promise.all(blocks
      ...
      .map(async (block) => {
        return {
          id: block.id, children: await getBlocks(block.id),
        };
      }));
  const blocksWithChildren = blocks.map((block) => {
    // Add child blocks if the block should contain children but none exists
    if (block.has_children && !block[block.type].children) {
      block[block.type]["children"] = childBlocks.find((x) => x.id === block.id
      )?.children;
    }
    return block;
  });
